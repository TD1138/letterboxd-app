...
imports_end = time.time()
imports = imports_end - imports_start

st.set_page_config(layout="wide")

if 'dfs' not in st.session_state:

    initial_data_load_start = time.time()
    ...
    initial_data_load_end = time.time()
    initial_data_load = initial_data_load_end - initial_data_load_start
    cached_data_load = None

else:
    
    cached_data_load_start = time.time()
    ...
    cached_data_load_end = time.time()
    cached_data_load = cached_data_load_end - cached_data_load_start
...
sidebar_filtering_start = time.time()
...
sidebar_filtering_end = time.time()
sidebar_filtering = sidebar_filtering_end - sidebar_filtering_start
...
with watchlist_tab:
    ...
    watchlist_tab_end = time.time()
    watchlist_tab = watchlist_tab_end - watchlist_tab_start

with diary_tab:
    diary_tab_start = time.time()
    ...
    st.dataframe(diary_detail_df.head(100), use_container_width=True, hide_index=True)
    diary_tab_end = time.time()
    diary_tab = diary_tab_end - diary_tab_start
...
with stats:
    ...
    stats_tab_end = time.time()
    stats_tab = stats_tab_end - stats_tab_start

with year_tab:
    year_tab_start = time.time()
    ...
    year_tab_end = time.time()
    year_tab = year_tab_end - year_tab_start
        
with genre_tab:
    genre_tab_start = time.time()
    ...
    genre_tab_end = time.time()
    genre_tab = genre_tab_end - genre_tab_start
	
with director_tab:
    director_tab_start = time.time()
    ...
    director_tab_end = time.time()
    director_tab = director_tab_end - director_tab_start

with actor_tab:
    actor_tab_start = time.time()    
    ...
    actor_tab_end = time.time()
    actor_tab = actor_tab_end - actor_tab_start

with collections_tab:
    collections_tab_start = time.time()
    ...
    collections_tab_end = time.time()
    collections_tab = collections_tab_end - collections_tab_start

with filmid_lookup_tab:
    filmid_lookup_tab_start = time.time()
    ...
    filmid_lookup_tab_end = time.time()
    filmid_lookup_tab = filmid_lookup_tab_end - filmid_lookup_tab_start

end = time.time()

total_seconds = end - start

print('Total time taken was {} seconds.'.format(total_seconds))
...
try:
    timings_df = pd.read_csv('timings_output.csv')
    run_num = 1+timings_df['run_num'].max()
except:
    timings_df = pd.DataFrame(columns=columns_df)
    run_num = 1

run_info = 'limit diary to last 100 films'
valid_run = True
...
if valid_run:
    ...
    row_df = pd.DataFrame(row, columns = columns_df)

    timings_df = pd.concat([timings_df, row_df], axis=0)

    timings_df.to_csv('timings_output.csv', index=False)
    print('Total runs = {}'.format(int(timings_df['run_num'].max())))
    print(timings_df.drop(['run_num'], axis=1).groupby('run_info').mean())
